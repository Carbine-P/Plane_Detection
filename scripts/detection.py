...
latest_corners = {
    'corner1': (0.0, 0.0, 0.0),  # 默认值，将被平面检测结果覆盖
    'corner2': (0.0, 0.0, 0.0),
    'corner3': (0.0, 0.0, 0.0),
    'corner4': (0.0, 0.0, 0.0)
}
...
# --- 静态角点数据 ---
# 默认角点坐标 (单位: mm)
STATIC_NEAR_LEFT = (0.0, 0.0, 0.0)
STATIC_NEAR_RIGHT = (0.0, 0.0, 0.0)
STATIC_FAR_RIGHT = (0.0, 0.0, 0.0)
STATIC_FAR_LEFT = (0.0, 0.0, 0.0)
...
def plane_detection_callback(msg):
    ...
    print(f"[平面检测回调] 收到ROS消息! 时间戳: {time.time()}")
    ...
    try:
        ...
        for point in msg.corners:
            # 确保使用float32格式
            x_mm = float(point.x)  # 已经是毫米单位
            y_mm = float(point.y)
            z_mm = float(point.z)
            corners.append((x_mm, y_mm, z_mm))
...
def setup_ros():
    ...
    try:
        ...
        subscriber = rospy.Subscriber(plane_detection_topic, PlaneCorners, plane_detection_callback)
        
        print(f"[ROS] 成功订阅 '{plane_detection_topic}'")
        print(f"[ROS] 订阅者对象: {subscriber}")
        print(f"[ROS] 等待话题发布者...")
        
        # 等待话题发布者连接
        timeout = 10.0
        start_time = time.time()
        while subscriber.get_num_connections() == 0 and (time.time() - start_time) < timeout:
            print(f"[ROS] 等待发布者连接 '{plane_detection_topic}'... ({time.time() - start_time:.1f}s)")
            time.sleep(1.0)
        
        if subscriber.get_num_connections() > 0:
            print(f"[ROS] 已连接到 {subscriber.get_num_connections()} 个发布者")
        else:
            print(f"[ROS] 警告: {timeout}秒内未检测到发布者")
        return True
    except Exception as e:
            ...
            try:
                ...
                if response_packet:
                    ...
                    try:
                        ...
                        if use_dynamic:
                            print(f"  角点1: [{current_corners['corner1'][0]:.1f}, {current_corners['corner1'][1]:.1f}, {current_corners['corner1'][2]:.1f}] mm (动态)")
                            print(f"  角点2: [{current_corners['corner2'][0]:.1f}, {current_corners['corner2'][1]:.1f}, {current_corners['corner2'][2]:.1f}] mm (动态)")
                            print(f"  角点3: [{current_corners['corner3'][0]:.1f}, {current_corners['corner3'][1]:.1f}, {current_corners['corner3'][2]:.1f}] mm (动态)")
                            print(f"  角点4: [{current_corners['corner4'][0]:.1f}, {current_corners['corner4'][1]:.1f}, {current_corners['corner4'][2]:.1f}] mm (动态)")
                        else:
                            print(f"  角点1: [{STATIC_NEAR_LEFT[0]:.1f}, {STATIC_NEAR_LEFT[1]:.1f}, {STATIC_NEAR_LEFT[2]:.1f}] mm (静态)")
                            print(f"  角点2: [{STATIC_NEAR_RIGHT[0]:.1f}, {STATIC_NEAR_RIGHT[1]:.1f}, {STATIC_NEAR_RIGHT[2]:.1f}] mm (静态)")
                            print(f"  角点3: [{STATIC_FAR_RIGHT[0]:.1f}, {STATIC_FAR_RIGHT[1]:.1f}, {STATIC_FAR_RIGHT[2]:.1f}] mm (静态)")
                            print(f"  角点4: [{STATIC_FAR_LEFT[0]:.1f}, {STATIC_FAR_LEFT[1]:.1f}, {STATIC_FAR_LEFT[2]:.1f}] mm (静态)")
