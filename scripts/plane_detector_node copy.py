#!/home/orin/anaconda3/envs/yolov11/bin/python
import rospy
import cv2
import numpy as np
...
class PlaneDetector:
    def __init__(self):
        ...
        # 加载相机参数
        camera_json_path = rospy.get_param('~camera_json_path', '/home/orin/PHT/PosDet/data/camera.json')
        ...
        # 处理控制参数
        self.process_interval = rospy.get_param('~process_interval', 1.0)  # 处理间隔（秒）
        ...
        self.output_unit = rospy.get_param('~output_unit', 'centimeter')  # 'meter' 或 'centimeter'
        self.unit_scale = 100.0 if self.output_unit == 'centimeter' else 1.0  # 单位转换因子
    ...
    def image_callback(self, rgb_msg, depth_msg):
        ...
        try:
            ...
            # 原点平移向量（单位：米）
            origin_shift = np.array([0.043, 0.050, 0.0])  # 右 4.3cm，下 5cm

            # 对相机坐标下的角点进行原点平移（单位为米）
            corners_camera_shifted = [np.array(corner) - origin_shift for corner in corners_camera]

            # 将平移后的角点转换为目标坐标系（含单位转换，输出单位：厘米）
            corners_shifted_converted = self.convert_coordinate_system(corners_camera_shifted)
            ...
            for i, corner in enumerate(corners_shifted_converted):
                print(f"  {corner_labels[i]}: [{corner[0]:.1f}, {corner[1]:.1f}, {corner[2]:.1f}] cm")
            ...
            msg.corners = [Point(x=float(c[0]), y=float(c[1]), z=float(c[2])) for c in corners_shifted_converted]
    ...
    def create_corner_visualization(self, rgb, corners_3d_camera, corners_3d_converted):
        ...
        # 相对于 O' 的角点坐标（用于文本显示）
        corner_points_3d_camera_relative = [np.array(corner) - origin_shift for corner in corners_3d_camera]

        # 相对坐标再做坐标系转换 + 单位变换（用于显示）
        corners_3d_converted_relative = self.convert_coordinate_system(corner_points_3d_camera_relative)
        ...
        try:
            ...
            # 绘制角点（在连线上方）
            corner_labels = ["LN", "RN", "RF", "LF"]  # (左)近距点, (右)近距点, (右)远距点, (左)远距点
            ...
            # 添加信息文本（显示转换后的坐标，单位为厘米）
            # 替换信息文本输出部分
            info_text = [
                "Plane Detection - 4 Corners (relative to shifted origin O')",
                f"(left)near: ({corners_3d_converted_relative[0][0]:.1f}, {corners_3d_converted_relative[0][1]:.1f}, {corners_3d_converted_relative[0][2]:.1f})cm",
                f"(right)near: ({corners_3d_converted_relative[1][0]:.1f}, {corners_3d_converted_relative[1][1]:.1f}, {corners_3d_converted_relative[1][2]:.1f})cm",
                f"(right)far: ({corners_3d_converted_relative[2][0]:.1f}, {corners_3d_converted_relative[2][1]:.1f}, {corners_3d_converted_relative[2][2]:.1f})cm",
                f"(left)far: ({corners_3d_converted_relative[3][0]:.1f}, {corners_3d_converted_relative[3][1]:.1f}, {corners_3d_converted_relative[3][2]:.1f})cm"
            ]
            ...
            # 添加尺寸信息
            size_text = f"Size: {avg_width:.1f}cm x {avg_height:.1f}cm"
    ...
    def generate_mask(self, rgb):
        ...
        results = self.model.predict(rgb, imgsz=640, half=True, verbose=False, conf=self.confidence_threshold)
    ...
    def process_images(self, rgb, depth, mask):
        ...
        rospy.loginfo(f"拟合平面方程: {a:.4f}x + {b:.4f}y + {c:.4f}z + {d:.4f} = 0")
        ...
        try:
            ...
            for corner in corners_camera:
                ...
                if self.coordinate_system == 'right_hand':
                    ...
                    # 保持相机坐标系
                    x_new, y_new, z_new = x_cam, y_cam, z_cam
                
                # 应用单位转换
                x_new *= self.unit_scale
                y_new *= self.unit_scale  
                z_new *= self.unit_scale
                
                converted_corners.append([float(x_new), float(y_new), float(z_new)])
    ...
    def sort_corners(self, corners_3d):
        """
        对四个角点进行排序，确保固定的顺序：
        P1: （左）近距点 (Left-Near)
        P2: （右）近距点 (Right-Near) 
        ...
        try:
            ...
            for corner_3d in corners_3d:
                ...
                if z_3d > 0:  # 确保深度有效
                    u = int(x_3d * self.K[0, 0] / z_3d + self.K[0, 2])
                    v = int(y_3d * self.K[1, 1] / z_3d + self.K[1, 2])
                    corners_2d.append((u, v, corner_3d))
                else:
                    # 如果深度无效，使用默认位置
                    corners_2d.append((320, 240, corner_3d))
            
            # 按照图像坐标进行排序
            # 1. 按Y坐标排序，分为上下两组
            corners_2d.sort(key=lambda x: x[1])  # 按v坐标(y)排序
            top_corners = corners_2d[:2]    # 上面两个点
            bottom_corners = corners_2d[2:]  # 下面两个点
            
            # 2. 在每组内按X坐标排序
            top_corners.sort(key=lambda x: x[0])     # 按u坐标(x)排序  
            bottom_corners.sort(key=lambda x: x[0])   # 按u坐标(x)排序
            
            # 3. 提取排序后的3D角点：左近、右近、右远、左远
            sorted_corners = [
                bottom_corners[1][2],   # P3: 顺时针（右）远距点 (Right-Far)
                bottom_corners[0][2],   # P4: 顺时针（左）远距点 (Left-Far)
                top_corners[1][2],      # P2: 顺时针（右）近距点 (Right-Near)
                top_corners[0][2]       # P1: 顺时针（左）近距点 (Left-Near)
            ]
            
            corner_names = ["（左）近距点", "（右）近距点", "（右）远距点", "（左）远距点"]
            # 角点排序完成（调试信息已简化）
